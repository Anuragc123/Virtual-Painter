...
class VideoCamera():
    ...
    def get_frame(self, overlay_image):
        ...
        # Check if the user wants to draw a circle
            if my_fingers[0] and my_fingers[1]:
                self.xp, self.yp = 0,0
                self.draw_circle = True
                self.draw_rectangle = False

        # Check if the user wants to draw a rectangle
            if my_fingers[0] and my_fingers[2]:
                self.xp, self.yp = 0,0
            ...
            if self.draw_circle:
            # Draw a circle
                cv2.circle(self.image_canvas, (self.x1, self.y1), 20, self.draw_color, -1)
                cv2.circle(frame, (self.x1, self.y1), 20, self.draw_color, -1)
            elif self.draw_rectangle:
            # Draw a rectangle
                cv2.rectangle(self.image_canvas, (self.x1 - 20, self.y1 - 20), (self.x1 + 20, self.y1 + 20), self.draw_color, -1)
                cv2.rectangle(frame, (self.x1 - 20, self.y1 - 20), (self.x1 + 20, self.y1 + 20), self.draw_color, -1)
